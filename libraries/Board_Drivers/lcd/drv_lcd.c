...
#define LCD_CLEAR_SEND_NUMBER 5760
    ...
    #define PWM_BL_NAME        "pwm14"              /* 鑳屽厜PWM璁惧鍚嶇О */
    #define PWM_BL_CHANNEL     1                    /* 鑳屽厜PWM閫氶亾 */
    #define PWM_BL_PERIOD      50000                /* 0.05ms = 20000 Hz*/
    struct rt_device_pwm *pwm_bl_dev = RT_NULL;     /* PWM璁惧鍙ユ焺 */
#else
...
{
    LCD->_u8_REG = regval; // 鍐欏叆瑕佸啓鐨勫瘎瀛樺櫒搴忓彿
}
// 鍐橪CD鏁版嵁
// data:瑕佸啓鍏ョ殑鍊?
...
// 鍐欏瘎瀛樺櫒
// LCD_Reg:瀵勫瓨鍣ㄥ湴鍧€
// LCD_RegValue:瑕佸啓鍏ョ殑鏁版嵁
void LCD_WriteReg(uint8_t LCD_Reg, uint16_t LCD_RegValue)
{
    LCD->_u8_REG = LCD_Reg;      // 鍐欏叆瑕佸啓鐨勫瘎瀛樺櫒搴忓彿
    LCD->_u16_RAM = LCD_RegValue; // 鍐欏叆鏁版嵁
}
// 璇诲瘎瀛樺櫒
// LCD_Reg:瀵勫瓨鍣ㄥ湴鍧€
// 杩斿洖鍊?璇诲埌鐨勬暟鎹?
...
{
    LCD_WR_REG(LCD_Reg);  // 鍐欏叆瑕佽鐨勫瘎瀛樺櫒搴忓彿
    return LCD_RD_DATA8(); // 杩斿洖璇诲埌鐨勫€?
}
// 寮€濮嬪啓GRAM
...
{
    LCD->_u16_RAM = RGB_Code; // 鍐欏崄鍏綅GRAM
}
...
void LCD_ReadPoint(char *pixel, int x, int y)
{
    ...
    {
        *color = 0; // 瓒呰繃浜嗚寖鍥?鐩存帴杩斿洖
        return;
    }
    LCD_SetCursor(x, y);

    LCD_WR_REG(0X2E); // 9341/3510/1963 鍙戦€佽GRAM鎸囦护

    r = LCD_RD_DATA8();      // dummy Read
    r = LCD_RD_DATA8(); // 瀹為檯鍧愭爣棰滆壊
    b = LCD_RD_DATA8();

    g = r & 0XFF; // 瀵逛簬9341/5310/5510,绗竴娆¤鍙栫殑鏄疪G鐨勫€?R鍦ㄥ墠,G鍦ㄥ悗,鍚勫崰8浣?
    g <<= 8;
    *color = (((r >> 11) << 11) | ((g >> 10) << 5) | (b >> 11)); // ILI9341/NT35310/NT35510闇€瑕佸叕寮忚浆鎹竴涓?
}
// LCD寮€鍚樉绀?
...
// 璁剧疆LCD鐨勮嚜鍔ㄦ壂鎻忔柟鍚?瀵筊GB灞忔棤鏁?
// 娉ㄦ剰:鍏朵粬鍑芥暟鍙兘浼氬彈鍒版鍑芥暟璁剧疆鐨勫奖鍝?灏ゅ叾鏄?341),
...
void lcd_clear(rt_uint16_t color)
{
    ...
    lcd_address_set(0, 0, lcddev.width - 1, lcddev.height - 1); // 璁剧疆鍏夋爣浣嶇疆
...
rt_err_t drv_lcd_init(struct rt_device *device)
{
    ...
    read_timing.AddressSetupTime = 0XF;  //鍦板潃寤虹珛鏃堕棿锛圓DDSET锛変负16涓狧CLK 1/168M=6ns*16=96ns
    read_timing.AddressHoldTime = 0x00;  //鍦板潃淇濇寔鏃堕棿锛圓DDHLD锛夋ā寮廇鏈敤鍒?
    read_timing.DataSetupTime = 60;         //鏁版嵁淇濆瓨鏃堕棿涓?0涓狧CLK    =6*60=360ns
    read_timing.BusTurnAroundDuration = 0x00;
    read_timing.CLKDivision = 0x00;
    read_timing.DataLatency = 0x00;
    read_timing.AccessMode = FSMC_ACCESS_MODE_A;     //妯″紡A


    write_timing.AddressSetupTime =9;         //鍦板潃寤虹珛鏃堕棿锛圓DDSET锛変负9涓狧CLK =54ns
    write_timing.AddressHoldTime = 0x00;     //鍦板潃淇濇寔鏃堕棿锛圓
    write_timing.DataSetupTime = 8;      //鏁版嵁淇濆瓨鏃堕棿涓?ns*9涓狧CLK=54ns
    write_timing.BusTurnAroundDuration = 0x00;
    write_timing.CLKDivision = 0x00;
    write_timing.DataLatency = 0x00;
    write_timing.AccessMode = FSMC_ACCESS_MODE_A;    //妯″紡A
    ...
    LOG_I(" LCD ID:%x", lcddev.id); // 鎵撳嵃LCD ID
    ...
    LCD_WR_REG(0x21);       // 寮€鍚弽鑹?
#ifdef BSP_USING_ONBOARD_LCD_TEAREFFECT
    /* TearEffect Sync On */
    LCD_WR_REG(0x35);       // 寮€鍚疶E
    LCD_WR_DATA8(0x00);     // TE 鍚屾鏂瑰紡锛歷sync 鍚屾
#endif /* BSP_USING_ONBOARD_LCD_TEAREFFECT */
    /* Sleep Out */
    LCD_WR_REG(0x11);
    ...
    // 鍒濆鍖栧畬鎴愪互鍚?鎻愰€?
    // st7789v3鍙互璁剧疆WR鏃跺簭涓烘渶蹇?
    {
        // 閲嶆柊閰嶇疆鍐欐椂搴忔帶鍒跺瘎瀛樺櫒鐨勬椂搴?
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 0); // 鍦板潃寤虹珛鏃堕棿(ADDSET)娓呴浂
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 8); // 鏁版嵁淇濆瓨鏃堕棿娓呴浂
        FSMC_Bank1E->BWTR[6] |= 3 << 0;      // 鍦板潃寤虹珛鏃堕棿(ADDSET)涓?涓狧CLK =18ns
        FSMC_Bank1E->BWTR[6] |= 2 << 8;      // 鏁版嵁淇濆瓨鏃堕棿(DATAST)涓?ns*3涓狧CLK=18ns
    }
    LCD_Display_Dir(0); // 榛樿涓烘í灞?
    lcd_clear(WHITE);
    ...
    LCD_BackLightSet(80);
#else
    rt_pin_write(LCD_BL, PIN_HIGH); // 寮€鍚儗鍏?
#endif /* BSP_USING_ONBOARD_LCD_PWM_BL */
